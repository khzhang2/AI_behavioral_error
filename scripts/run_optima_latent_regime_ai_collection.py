...
CHOICE_CODE_TO_NAME = {value: key for key, value in CHOICE_NAME_TO_CODE.items()}
...
def previous_answer_strings(attitude_rows: list[dict], task_rows: list[dict]) -> list[str]:
    ...
    for row in task_rows:
        if int(row["is_valid_task_response"]) == 1:
            history.append(
                f"T{int(row['task_index'])}={row['choice_label']}/{row['chosen_alternative_name']}/conf={int(row['confidence'])}"
            )
    return history
...
def build_ai_panel_block(block_frame: pd.DataFrame, response_frame: pd.DataFrame) -> pd.DataFrame:
    records: list[dict] = []
    response_lookup = {
        (str(row["respondent_id"]), int(row["task_index"])): row for _, row in response_frame.iterrows()
    }
    for _, block in block_frame.iterrows():
        respondent_id = str(block["respondent_id"])
        rows = response_frame.loc[response_frame["respondent_id"] == respondent_id].copy()
        rows = rows.sort_values("task_index").reset_index(drop=True)
        ...
        for _, dom_row in rows.loc[rows["task_role"] == "dominance"].iterrows():
            target_code = CHOICE_NAME_TO_CODE.get(str(dom_row["dominated_alternative_name"]), -1)
            if int(dom_row["is_valid_task_response"]) == 1:
                dominance_values.append(int(int(dom_row["choice_code"]) == target_code))
...
def finalize_outputs(base_dir: Path, model_key: str, block_frame: pd.DataFrame, task_frame: pd.DataFrame) -> None:
    attitudes_path = base_dir / "parsed_attitudes.csv"
    tasks_path = base_dir / "parsed_task_responses.csv"
    attitudes = pd.read_csv(attitudes_path) if attitudes_path.exists() and attitudes_path.stat().st_size > 0 else pd.DataFrame()
...
def main() -> None:
    ...
    for _, persona in block_frame.iterrows():
        ...
        system_prompt = build_system_prompt(persona.to_dict(), str(persona["prompt_arm"]))
        ...
        update_progress(base_dir, CONFIG["experiment_name"], len(block_frame), len(completed))
        valid_attitudes = sum(int(row["is_valid_indicator"]) for row in attitude_rows)
        valid_tasks = sum(int(row["is_valid_task_response"]) for row in task_rows)
        print(
            f"[latent_regime_ai_collection] model={args.model_key} respondent={respondent_id} "
            f"valid_attitudes={valid_attitudes}/{len(attitude_rows)} valid_tasks={valid_tasks}/{len(task_rows)}"
        )

    finalize_outputs(base_dir, args.model_key, block_frame, task_frame)
