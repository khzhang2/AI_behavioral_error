...
def main() -> None:
    ...
    result = minimize(
        ...
        options={
            "maxiter": int(CONFIG["salcm"]["maxiter"]),
            "maxfun": int(CONFIG["salcm"].get("maxfun", 100000)),
            "ftol": float(CONFIG["salcm"].get("ftol", 1e-10)),
        },
    )
    ...
    regime_frame.to_csv(output_dir / "ai_salcm_regime_summaries.csv", index=False)
    ...
    pd.DataFrame(block_scores).to_csv(output_dir / "ai_salcm_block_distortion_scores.csv", index=False)
    ...
    write_json(output_dir / "ai_salcm_summary.json", summary)
    print(
