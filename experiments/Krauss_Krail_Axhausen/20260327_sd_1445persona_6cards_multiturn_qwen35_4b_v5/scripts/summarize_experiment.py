...
def main() -> None:
    raw_rows = [json.loads(line) for line in open(OUTPUT_DIR / "raw_interactions.jsonl") if line.strip()]
    ...
    lines = [
        ...
        f"- 平均每题调用时长：`{sum(choice_call_durations) / len(choice_call_durations):.2f}` 秒",
