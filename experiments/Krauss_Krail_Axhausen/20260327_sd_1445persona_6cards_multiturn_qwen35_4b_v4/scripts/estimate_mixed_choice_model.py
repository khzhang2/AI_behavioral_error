...
# Windows Anaconda + torch often loads two OpenMP runtimes; keep the
# experiment process alive instead of failing before estimation starts.
...
def prepare_tensors(expected_tasks_per_respondent: int) -> tuple[dict[str, torch.Tensor], list[int], int, int]:
    ...
    total_valid_respondents = int(data["respondent_id"].nunique())

    completed_respondents = (
        data.groupby("respondent_id")
        .size()
        .loc[lambda series: series == expected_tasks_per_respondent]
        .index.tolist()
    )
    data = data.loc[data["respondent_id"].isin(completed_respondents)].copy()
    data = data.sort_values(["respondent_id", "task_index"]).reset_index(drop=True)

    respondent_ids = sorted(data["respondent_id"].unique().tolist())
    tasks_per_respondent = expected_tasks_per_respondent
    n_respondents = len(respondent_ids)
    if n_respondents == 0:
        raise ValueError("No respondents with a complete set of valid choice tasks are available for estimation.")
    ...
    tensors = {
        "choice": torch.tensor(
            data["choice"].to_numpy(dtype=int).reshape(n_respondents, tasks_per_respondent) - 1,
            dtype=torch.long,
        ),
    ...
    dropped_incomplete_respondents = total_valid_respondents - len(completed_respondents)
    return tensors, respondent_ids, total_valid_respondents, dropped_incomplete_respondents
...
def main() -> None:
    ...
    expected_tasks_per_respondent = int(config["tasks_per_respondent"])
    tensors, respondent_ids, total_valid_respondents, dropped_incomplete_respondents = prepare_tensors(
        expected_tasks_per_respondent
    )
    ...
    summary = {
        ...
        "n_respondents": len(respondent_ids),
        "expected_tasks_per_respondent": expected_tasks_per_respondent,
        "total_valid_respondents_before_panel_filter": total_valid_respondents,
        "dropped_incomplete_respondents": dropped_incomplete_respondents,
