...
persona 支持集直接来自 `human_respondent_profiles.csv`。在 prepare 脚本中，这张表被读入后，只保留一组固定字段，形成 `profile_bank`，并写入实验目录中的 `respondent_profile_bank.csv`。当前默认还会保留一个唯一的 `source_observation_id`，它对应原始 `optima.dat` 里的 loop-level 行号，用来把 AI run 和人类 source observation 一一对齐。
...
| 行为与态度字段 | `Envir01`、`Envir02`、`Envir05`、`Envir06`、`Mobil10`、`Mobil11`、`Mobil16` | 作为 Atasoy 2011 exact HCM 所需的七个 attitude indicators 保存，供后续实验整理与对照使用 |
